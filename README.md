...
- React dashboard (an agent overview with real per-agent SQL rollups, trace explorer, trace/span detail with a real proportional-width waterfall and an alternate call-tree graph view, sessions, analytics, regression, alerts, and a live agent-connection page) wired entirely to the real collector API — no mock data left in the shipped app
...
- Annotate any trace good/bad with an optional note (human feedback, the pattern Langfuse Scores / LangSmith Feedback both ship), and promote a real captured trace straight into a paste-ready `iris-eval` test case — the collector doesn't own your suite.yaml, so this generates a snippet to copy in rather than pretending to write a file it can't reach, and is honest when a trace has no captured content to promote from
- Threshold alerting (Datadog Monitors) on error rate, P95 latency, or cost over a rolling window: a real in-process background loop on the collector (not a cron job or a client-side poll) evaluates every enabled rule against the same aggregate queries the dashboard itself reads, and fires a webhook once per breach window
