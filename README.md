...
- React dashboard (an agent overview with real per-agent SQL rollups, trace explorer, trace/span detail with a real proportional-width waterfall and an alternate call-tree graph view, analytics, regression, alerts, and a live agent-connection page) wired entirely to the real collector API — no mock data left in the shipped app
